...
class _IlanOlusturPageState extends State<IlanOlusturPage> {
  ...
  final List<File> _secilenResimler = [];
  ...
  Future<void> _resimEkle() async {
    if (_secilenResimler.length >= _maxResim) {
      _snackbar('En fazla $_maxResim resim ekleyebilirsiniz.', hata: true);
      return;
    }
    ...
    if (picked != null) {
      setState(() => _secilenResimler.add(File(picked.path)));
    }
  }
 
  void _resimSil(int index) {
    setState(() => _secilenResimler.removeAt(index));
  }
  ...
  Future<void> _ilanOlustur() async {
    ...
    try {
      ...
      if (_tip == 1 && _secilenResimler.isNotEmpty) {
        for (int i = 0; i < _secilenResimler.length; i++) {
          final ref = FirebaseStorage.instance
              .ref()
              .child('ilan_resimleri')
              .child(
                  '${user.uid}_${DateTime.now().millisecondsSinceEpoch}_$i.jpg');
          await ref.putFile(_secilenResimler[i]);
      ...
      await FirebaseFirestore.instance.collection('ilanlar').add({
        ...
        'kategori': _secilenKategori ?? 'diger',
        ...
        if (resimUrller.isNotEmpty) 'resimUrl': resimUrller.first,
      ...
      if (mounted) _snackbar('Hata: $e', hata: true);
    } finally {
  ...
  void _snackbar(String mesaj, {bool hata = false}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(mesaj, style: GoogleFonts.dmSans(color: Colors.white)),
                    ...
                    if (_tip == 1) ...[
                      ...
                      Wrap(
                        ...
                        children: kKategoriler.entries.map((e) {
                          final secili = _secilenKategori == e.key;
                      ...
                      Row(
                        children: [
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              color: _secilenResimler.length == _maxResim
                                  ? const Color(0xFFFFEBEE)
                                  : const Color(0xFFE8F5E9),
                              borderRadius: BorderRadius.circular(10),
                            ),
                            child: Text(
                              '${_secilenResimler.length}/$_maxResim',
                              style: GoogleFonts.dmSans(
                                  fontSize: 11,
                      ...
                      SizedBox(
                        ...
                        child: ListView(
                          ...
                          children: [
                            ..._secilenResimler.asMap().entries.map((e) {
                              final index = e.key;
                              final file = e.value;
                              return Padding(
...
const List<String> _sehirler = [
  ...
  'Birmingham (İngiltere)', 'Edinburgh (İngiltere)', 'Bristol (İngiltere)',
  ...
  'Sevilla (İspanya)', 'Roma (İtalya)', 'Milano (İtalya)',
  'Napoli (İtalya)', 'Venedik (İtalya)', 'Floransa (İtalya)',
  ...
  'Johannesburg (Güney Afrika)', 'Cape Town (Güney Afrika)', 'Accra (Gana)',
